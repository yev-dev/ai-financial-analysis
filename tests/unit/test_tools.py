...
from fin_ai.core import service as svc
from fin_ai.core import tools


class _FakeOnlineService:
    """In-memory fake of OnlineMarketDataService used to test tools.py dispatch."""
    ...
    def get_income_stmt(self, symbol: str) -> pd.DataFrame:
        return pd.DataFrame({"2025": [1000]}, index=["Revenue"])

    def get_balance_sheet(self, symbol: str) -> pd.DataFrame:
        return pd.DataFrame({"2025": [500]}, index=["Assets"])

    def get_cash_flow(self, symbol: str) -> pd.DataFrame:
        return pd.DataFrame({"2025": [120]}, index=["Operating Cash Flow"])
...
def fake_service(monkeypatch):
    """Replace the cached service in tools with an in-memory fake."""
    monkeypatch.setattr(tools, "_market_service", _FakeOnlineService())


class TestLiteLlmToolFunctions:
    @pytest.mark.unit
    def test_get_stock_data_returns_json_serializable_payload(self, fake_service):
    ...
    @pytest.mark.unit
    def test_get_analyst_recommendations_returns_consensus(self, fake_service):
...
class TestLiteLlmToolRegistry:
    @pytest.mark.unit
    def test_tool_schemas_are_subset_of_registered_functions(self):
        schema_names = {entry["function"]["name"] for entry in tools.YAHOO_FINANCE_TOOLS}
        registered_names = set(tools.LITELLM_TOOL_FUNCTIONS)

        assert schema_names.issubset(registered_names), (
            f"Schema has tools not in registry: {schema_names - registered_names}"
        )

    @pytest.mark.unit
    def test_yahoo_schema_names_are_8(self):
        assert len(tools.YAHOO_FINANCE_TOOLS) == 8

    @pytest.mark.unit
    def test_execute_litellm_tool_call_dispatches_and_handles_errors(self, fake_service):
